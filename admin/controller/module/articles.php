...
class ControllerModuleArticles extends Controller {
	...
	public function install() {
		$this->db->query("CREATE TABLE IF NOT EXISTS `" . DB_PREFIX . "articles` (
		...
		$this->db->query("CREATE TABLE IF NOT EXISTS `" . DB_PREFIX . "articles_description` (
		  `articles_description_id` int(11) NOT NULL AUTO_INCREMENT,
		  `articles_id` int(11) NOT NULL,
		  `language_id` int(11) NOT NULL,
		  `title` varchar(255) COLLATE utf8_bin NOT NULL,
		  `description` text COLLATE utf8_bin NOT NULL,
		  `short_description` text COLLATE utf8_bin NOT NULL,
		  PRIMARY KEY (`articles_description_id`)
		)");
		
		$this->load->model('user/user_group');

		$this->model_user_user_group->addPermission($this->user->getId(), 'access', 'extension/articles');
		$this->model_user_user_group->addPermission($this->user->getId(), 'modify', 'extension/articles');
	}
